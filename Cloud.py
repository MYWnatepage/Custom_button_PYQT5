...
class Clouds():
    def __init__(self, x, y, radius, color, shift, random):
        ...
        # ---------------组成圆形---------------
        self.clouds = [
            {"x": 1.30 * x, "y": (2.53 * y + self.shift) * self.random, "radius": 0.92 * radius},
            {"x": 1.90 * x, "y": (2.40 * y + self.shift) * self.random, "radius": 1.05 * radius},
            {"x": 2.79 * x, "y": (2.13 * y + self.shift) * self.random, "radius": 1.03 * radius},
            {"x": 3.64 * x, "y": (2.19 * y + self.shift) * self.random, "radius": 0.95 * radius},
            {"x": 4.33 * x, "y": (1.72 * y + self.shift) * self.random, "radius": 0.75 * radius},
            {"x": 4.97 * x, "y": (1.37 * y + self.shift) * self.random, "radius": 0.97 * radius}
        ]
